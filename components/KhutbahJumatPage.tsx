...
import EditKhutbahJumatModal from './EditKhutbahJumatModal';
import AddKhutbahJumatModal from './AddKhutbahJumatModal';
import * as db from '../db';

interface KhutbahJumatPageProps {
    // Props removed as we fetch internally
}

const KhutbahJumatPage: React.FC<KhutbahJumatPageProps> = () => {
    const [works, setWorks] = useState<KhutbahJumat[]>([]);
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [editingWork, setEditingWork] = useState<KhutbahJumat | null>(null);
    ...
    const handleAddWork = async (newWorkData: Omit<KhutbahJumat, 'id'>) => {
        const addedWork = await db.addKhutbahJumat(newWorkData);
        if (addedWork) {
            setWorks([...works, addedWork]);
            setIsAddModalOpen(false);
            await db.addNotification({
                type: 'khutbah-jumat',
                title: "Khutbah Jum'at Baru Tersedia!",
                message: `Khutbah "${addedWork.judul}" telah ditambahkan. Silakan baca sekarang!`,
                timestamp: new Date(),
                isRead: false,
            });
    ...
    const handleUpdateWork = async (updatedWork: KhutbahJumat) => {
        const success = await db.updateKhutbahJumat(updatedWork);
        if (success) {
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 h-full flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center space-x-2">
                        <SermonIcon className="h-8 w-8 text-dark-teal" />
                        <h2 className="text-xl font-bold text-gray-800 uppercase tracking-wide">
                            DAFTAR KHUTBAH JUM'AT
                        </h2>
                    </div>
                    <button
                            ...
                            {works.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                                        {isLoading ? 'Loading...' : "Tidak ada data khutbah jum'at."}
                                    </td>
                                </tr>
                            )}
                        </tbody>
...
export default KhutbahJumatPage;