...
const navItems: NavItemType[] = [
  ...
  { id: 'buku-umum', label: 'Kelolah Buku Umum', icon: BookIcon },
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, onNavigate, hasUnreadMessages, onLogout }) => {
  ...
  return (
    <>
      ...
      ></div>
      <aside
        className={`transform top-0 left-0 w-64 bg-gray-50 text-gray-800 fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30 md:relative md:translate-x-0 md:w-72 flex-shrink-0 border-r border-gray-200 ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="p-4">
