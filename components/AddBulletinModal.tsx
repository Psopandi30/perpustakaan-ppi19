...
interface AddBulletinModalProps {
  onClose: () => void;
  onSave: (bulletin: Omit<Bulletin, 'id'>) => void;
}

const AddBulletinModal: React.FC<AddBulletinModalProps> = ({ onClose, onSave }) => {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="addIsFeaturedBulletin"
              name="isFeatured"
              checked={formData.isFeatured}
              onChange={handleChange}
              className="rounded border-gray-300 text-brand-yellow focus:ring-brand-yellow"
            />
            <label htmlFor="addIsFeaturedBulletin" className="text-sm text-gray-700 font-medium">
