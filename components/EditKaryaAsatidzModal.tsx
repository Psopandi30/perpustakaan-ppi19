...
interface EditKaryaAsatidzModalProps {
  work: KaryaAsatidz;
  onClose: () => void;
  onSave: (work: KaryaAsatidz) => void;
}

const EditKaryaAsatidzModal: React.FC<EditKaryaAsatidzModalProps> = ({ work, onClose, onSave }) => {
  const [formData, setFormData] = useState<KaryaAsatidz>({
    ...work,
    ...
    >
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b sticky top-0 bg-white">
          <h2 className="text-xl font-semibold text-gray-800">Edit Karya Ulama Persis</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {formFields.map(field => (
            <div key={field.name}>
              ...
              <input
                ...
                id={`edit-${field.name}`}
                name={field.name}
                value={String(formData[field.name as keyof Omit<KaryaAsatidz, 'id'>])}
          ...
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="editIsFeaturedAsatidz"
              checked={isFeatured}
              onChange={(e) => setIsFeatured(e.target.checked)}
              className="rounded border-gray-300 text-brand-yellow focus:ring-brand-yellow"
            />
            <label htmlFor="editIsFeaturedAsatidz" className="text-sm text-gray-700 font-medium">
...
export default EditKaryaAsatidzModal;
