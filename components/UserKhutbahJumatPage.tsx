...
import { SermonIcon, RadioIcon, HomeIcon, ExitIcon, BookOpenIcon, ArrowLeftIcon } from './icons/Icons';
import * as db from '../db';

interface UserKhutbahJumatPageProps {
    onBack: () => void;
}

const UserKhutbahJumatDetailPage: React.FC<{ work: KhutbahJumat; onBack: () => void }> = ({ work, onBack }) => {
        ...
        <div className="bg-gray-100 min-h-screen font-sans flex flex-col">
            <header className="bg-dark-teal text-white p-4 flex justify-between items-center sticky top-0 z-10">
                <div className="flex items-center space-x-2">
                    <SermonIcon className="w-7 h-7" />
                    <h1 className="text-xl font-semibold">Khutbah Jum'at</h1>
                </div>
...
const UserKhutbahJumatPage: React.FC<UserKhutbahJumatPageProps> = ({ onBack }) => {
    const [works, setWorks] = useState<KhutbahJumat[]>([]);
    const [selectedWork, setSelectedWork] = useState<KhutbahJumat | null>(null);
    ...
    if (selectedWork) {
        return <UserKhutbahJumatDetailPage work={selectedWork} onBack={() => setSelectedWork(null)} />;
    }

    const Card: React.FC<{ work: KhutbahJumat }> = ({ work }) => {
        const coverUrl = resolveImageUrl(work.coverLink);
        return (
        ...
        <div className="bg-gray-100 min-h-screen font-sans flex flex-col">
            ...
            <main className="flex-grow p-4 space-y-4 pb-24">
                <div className="space-y-3">
                    {isLoading ? (
                        <p className="text-center">Loading khutbah jumat...</p>
                    ) : works.length > 0 ? (
                        works.map(work => (
                            <Card key={work.id} work={work} />
                        ))
                    ) : (
                        <p className="text-gray-500 col-span-full text-center mt-8">Belum ada khutbah jumat yang dipublikasikan.</p>
                    )}
                </div>
            </main>
...
export default UserKhutbahJumatPage;