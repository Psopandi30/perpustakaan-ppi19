...
import { BookIcon, RadioIcon, HomeIcon, ExitIcon, BookOpenIcon, ArrowLeftIcon } from './icons/Icons';
import * as db from '../db';

interface UserGeneralBookPageProps {
    onBack: () => void;
}

const UserGeneralBookDetailPage: React.FC<{ work: GeneralBook; onBack: () => void }> = ({ work, onBack }) => {
...
const UserGeneralBookPage: React.FC<UserGeneralBookPageProps> = ({ onBack }) => {
    const [works, setWorks] = useState<GeneralBook[]>([]);
    const [selectedWork, setSelectedWork] = useState<GeneralBook | null>(null);
    ...
    if (selectedWork) {
        return <UserGeneralBookDetailPage work={selectedWork} onBack={() => setSelectedWork(null)} />;
    }

    const Card: React.FC<{ work: GeneralBook }> = ({ work }) => {
        const coverUrl = resolveImageUrl(work.coverLink);
        return (
        ...
        <div className="bg-gray-100 min-h-screen font-sans flex flex-col">
            ...
            <main className="flex-grow p-4 space-y-4 pb-24">
                <div className="space-y-3">
                    {isLoading ? (
                        <p className="text-center">Loading skripsi...</p>
                    ) : works.length > 0 ? (
                        works.map(work => (
                            <Card key={work.id} work={work} />
                        ))
                    ) : (
                        <p className="text-gray-500 col-span-full text-center mt-8">Belum ada skripsi yang dipublikasikan.</p>
                    )}
                </div>
            </main>
...
export default UserGeneralBookPage;