...
import { GraduationCapIcon, RadioIcon, HomeIcon, ExitIcon, BookOpenIcon, ArrowLeftIcon } from './icons/Icons';
import * as db from '../db';

interface UserKaryaAsatidzPageProps {
    onBack: () => void;
}

const UserKaryaAsatidzDetailPage: React.FC<{ work: KaryaAsatidz; onBack: () => void }> = ({ work, onBack }) => {
        ...
        <div className="bg-gray-100 min-h-screen font-sans flex flex-col">
            <header className="bg-dark-teal text-white p-4 flex justify-between items-center sticky top-0 z-10">
                <div className="flex items-center space-x-2">
                    ...
                    <h1 className="text-xl font-semibold">DAFTAR KARYA ULMA PERSIS</h1>
                </div>
...
const UserKaryaAsatidzPage: React.FC<UserKaryaAsatidzPageProps> = ({ onBack }) => {
    const [works, setWorks] = useState<KaryaAsatidz[]>([]);
    const [selectedWork, setSelectedWork] = useState<KaryaAsatidz | null>(null);
    ...
    if (selectedWork) {
        return <UserKaryaAsatidzDetailPage work={selectedWork} onBack={() => setSelectedWork(null)} />;
    }

    const Card: React.FC<{ work: KaryaAsatidz }> = ({ work }) => {
        const coverUrl = resolveImageUrl(work.coverLink);
        return (
        ...
        <div className="bg-gray-100 min-h-screen font-sans flex flex-col">
            ...
            <main className="flex-grow p-4 space-y-4 pb-24">
                <div className="space-y-3">
                    {isLoading ? (
                        <p className="text-center">Loading karya asatidz...</p>
                    ) : works.length > 0 ? (
                        works.map(work => (
                            <Card key={work.id} work={work} />
                        ))
                    ) : (
                        <p className="text-gray-500 col-span-full text-center mt-8">Belum ada karya ulama persis yang dipublikasikan.</p>
                    )}
                </div>
            </main>
...
export default UserKaryaAsatidzPage;